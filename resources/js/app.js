
/**
 * First we will load all of this project's JavaScript dependencies which
 * includes Vue and other libraries. It is a great starting point when
 * building robust, powerful web applications using Vue and Laravel.
 */

require('./bootstrap');

window.Vue = require('vue');

/**
 * The following block of code may be used to automatically register your
 * Vue components. It will recursively scan this directory for the Vue
 * components and automatically register them with their "basename".
 *
 * Eg. ./components/ExampleComponent.vue -> <example-component></example-component>
 */

Vue.component('add-treatment', require('./components/treatments/Add.vue'));
Vue.component('treatments-list', require('./components/treatments/List.vue'));
Vue.component('claim-treatments-list', require('./components/claims/GenerateTreatmentTable.vue'));
Vue.component('policy-change-status', require('./components/policies/ChangeStatus.vue'));

Vue.component('edit-price', require('./components/treatments/EditPrice.vue'));
// const files = require.context('./', true, /\.vue$/i)
// files.keys().map(key => Vue.component(key.split('/').pop().split('.')[0], files(key)))

/**
 * Next, we will create a fresh Vue application instance and attach it to
 * the page. Then, you may begin adding components to this application
 * or customize the JavaScript scaffolding to fit your unique needs.
 */

const app = new Vue({
    el: '#app',
    data() {
      return {
        isOrtho: null,
        isProsthesis: null,
        previusExtractions: null,
        test: '',
      }
    },
    created() {
      this.isOrtho = $('#is_ortho').prop('checked');
      this.isProsthesis = $('#replacement_prosthesis').prop('checked');
      this.previusExtractions = $('#replacement_prosthesis').prop('checked');
    },
    methods: {
      validateOrtho: function() {
          this.isOrtho = $('#is_ortho').prop('checked');
      },
      validateProsthesis: function() {
          this.isProsthesis = $('#replacement_prosthesis').prop('checked');
      },
      validatePreviusExtractions: function() {
          this.previusExtractions = $('#previusExtractions').prop('checked');
      },
      
    }
});
